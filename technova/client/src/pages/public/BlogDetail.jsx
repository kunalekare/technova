    ...
    <>
        ...
        <div className="container-max mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="max-w-4xl">
            ...
            <motion.h1
              ...
              className="text-4xl md:text-6xl lg:text-7xl font-display font-extrabold text-white leading-[1.1] mb-8 drop-shadow-2xl"
            >
              {post.title}
            </motion.h1>
            ...
            >
              <div className="w-14 h-14 rounded-full bg-white/20 backdrop-blur-md border border-white/30 flex items-center justify-center text-white text-xl font-bold shadow-lg">
                {post.author.avatar || post.author.name.charAt(0)}
              </div>
              <div>
                <div className="text-lg font-bold text-white">{post.author.name}</div>
                <div className="text-sm text-white/80">{post.author.role} • {post.date}</div>
              </div>
            </motion.div>
          </div>
        </div>
        ...
        <div className="container-max mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row gap-12 max-w-6xl mx-auto">
            ...
            <div className="flex-1 max-w-4xl">
              ...
              {/* Prose Content */}
              <div className="prose prose-invert prose-lg max-w-none prose-headings:font-display prose-headings:font-bold prose-a:text-primary-400 hover:prose-a:text-primary-300 prose-p:text-surface-300 prose-p:leading-relaxed prose-li:text-surface-300">
                {post.content.split('\n\n').map((paragraph, i) => {
                  if (paragraph.startsWith('**') && paragraph.endsWith('**')) {
                    return <h3 key={i} className="text-3xl text-white mt-16 mb-8">{paragraph.replace(/\*\*/g, '')}</h3>;
                  }
                  if (paragraph.startsWith('**') && paragraph.includes(':**')) {
                    const title = paragraph.match(/\*\*(.*?)\*\*/)?.[1] || '';
                    const rest = paragraph.replace(/\*\*.*?\*\*/, '');
                    return (
                      <div key={i} className="mt-10 mb-6 bg-surface-900/50 border border-white/5 p-6 md:p-8 rounded-[32px] shadow-lg">
                        <h3 className="text-2xl text-white mb-4 !mt-0">{title}</h3>
                        {rest && <p className="!mb-0 text-lg">{rest}</p>}
                      </div>
                    );
                  }
      ...
      <section className="py-24 bg-[#020617] border-t border-white/5">
        <div className="container-max mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
          <h2 className="text-3xl font-display font-bold text-white mb-10">More from {post.category}</h2>
